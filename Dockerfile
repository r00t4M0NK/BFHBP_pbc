...
RUN echo ACTION\=\$1 > $STARTUPDIR/myfirefoxsvc.sh && echo if \! \[ \-f "$HOME/.firefoxtmp" \]\; then >> $STARTUPDIR/myfirefoxsvc.sh && echo exit >> $STARTUPDIR/myfirefoxsvc.sh && echo fi >> $STARTUPDIR/myfirefoxsvc.sh && echo if \[ \"\$ACTION\" \= \"containered\" \]\; then >> $STARTUPDIR/myfirefoxsvc.sh && echo "cp $HOME/.config/xfce4/panel/launcher-19/firefox.desktop \$(grep Exec=exo /home/$USERVNC/.config/xfce4/panel/launcher-19/* | tail -1 | cut -d':' -f1)" >> $STARTUPDIR/myfirefoxsvc.sh && echo fi >> $STARTUPDIR/myfirefoxsvc.sh && echo fi >> $STARTUPDIR/myfirefoxsvc.sh && echo fi >> $STARTUPDIR/myfirefoxsvc.sh && chmod 755 $STARTUPDIR/myfirefoxsvc.sh
