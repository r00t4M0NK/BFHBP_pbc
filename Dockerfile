...
RUN echo '<html><head></head><body><table><tr><td><a href="https://testmypage.com">My URL</a></td></tr>' > $HOME/mytestpage.html && echo '<tr><td><a href="https://www.freeopenvpn.org">Free Open VPN Server</a> user=freeopenvpn</td></tr>' >> $HOME/mytestpage.html && echo '<tr><td><a href="https://www.showmyip.com">Test on the IP address</a></td></tr>' >> $HOME/mytestpage.html && echo '<tr><td><a href="https://www.browserling.com/tools/ntlm-hash">Test a NT Hash</a></td></tr>' >> $HOME/mytestpage.html && echo '<tr><td>&nbsp;</td></tr>' >> $HOME/mytestpage.html && echo '</body></html>' >> $HOME/mytestpage.html && chown $USERVNC:$USERVNC $HOME/mytestpage.html && echo "mkdir -p /root/.vpn; cd /root/.vpn; export VPN_PASSWORD=\$2; export CONFIG_FILE=\$1; export VPN_USER=freeopenvpn; bash -c \"openvpn --config '\"\$CONFIG_FILE\"' --auth-user-pass <(echo -e \"\$VPN_USER\"'\n''\"\$VPN_PASSWORD\"')\"" >> $STARTUPDIR/mystartopenvpn.sh && chmod 755 $STARTUPDIR/mystartopenvpn.sh
